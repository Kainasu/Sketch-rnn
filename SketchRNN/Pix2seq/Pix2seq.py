    ...
    def sample(self, data: torch.Tensor, temperature: float, filename: Optional[str] = None):
        ...
        with torch.no_grad():
            ...
            for i in range(longest_seq_len):
                ...
                dist, q_logits, state = self.decoder(data, z, data, longest_seq_len, state)
                # Sample a stroke
...
class Configs():
    ...
    def __init__(self):
        ...
        data_sets = ['owl.npz']
        percentage = 0.1
        datasets, png_paths = load_dataset('./', data_sets, percentage)
    ...
    def step(self, batch: Any, is_train=True):
        ...
        dist, q_logits, _ = self.decoder(inputs, z, data, self.max_seq_length, None)
