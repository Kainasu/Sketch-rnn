...
def copy_hparams(hparams):
  ...
  return get_hparams(**hparams.values())
...
class SketchRNN():
  ...
  def build_model(self, hps):
    ...
    encoder_lstm = LSTM(hps.enc_rnn_size,
                ...
                return_state=True)
    bidirectional = Bidirectional(encoder_lstm, name='bidirectional_1')
    ...
    last_h = concatenate([last_h_fw, last_h_bw], 1, name='concatenate_1')
    ...
    self.z_mean = Dense(hps.z_size,
                  ...
                  bias_initializer='zeros',
                  name='dense_1')(last_h) # (batch_size, z_size)
    self.z_presig = Dense(hps.z_size,
                  activation='linear',
                  use_bias=True,
                  kernel_initializer=normal_init,
                  bias_initializer='zeros',
                  name='dense_2')(last_h) # (batch_size, z_size)
    ...
    self.batch_z = Lambda(sampling,
                    output_shape=(hps.z_size,),
                    name='lambda_1')([self.z_mean, self.z_presig])
    ...
    # (batch_size, max_seq_len, 5)
    overlay_x = RepeatVector(hps.max_seq_len, name='repeat_vector_1')(self.batch_z)
    # (batch_size, max_seq_len, z_size)
    actual_input_x = concatenate([decoder_inputs, overlay_x], 2, name='concatenate_2')
    ...
    self.output_layer = Dense(self.n_out, activation='linear', use_bias=True, name='dense_4')
